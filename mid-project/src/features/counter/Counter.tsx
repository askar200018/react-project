...
export const Counter: React.FC<{ className: string }> = ({ className }) => {
  ...
  return (
    <div className={counterClasses}>
      <button className="counter__button" aria-label="Increment value" onClick={increment}>
        +
      </button>
      <span className="counter__label">{counter}</span>
      <button className="counter__button" aria-label="Decrement value" onClick={decrement}>
        -
      </button>
    </div>
  );
